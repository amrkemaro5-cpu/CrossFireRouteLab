...
public sealed partial class DashboardForm
{
    ...
    readonly Timer polishTimer = new() { Interval = 250 };
    readonly List<double> liveLatency = new();
    readonly List<double> liveJitter = new();
    readonly List<long> liveSamples = new();
    ...
    {
        if (center.Controls.Count < 2 || center.Controls[1] is not GRLCard summary) return;
        detectedGameIcon.Size = new Size(58, 58);
        detectedGameIcon.Location = new Point(20, 54);
    ...
    void ArrangeLeft(Control control)
    {
        ...
        all?.SetBounds(16, buttonY, w - 32, buttonHeight);
        clear?.SetBounds(16, buttonY + 48, w - 32, buttonHeight);
    ...
    void HookEndpointAutomation()
    {
        ...
        if (find != null)
        {
            find.Click += (_, _) => ScheduleEndpointSelection();
        }
        var auto = actions.FirstOrDefault(x => x.Text == "AUTO ANALYZE");
        if (auto != null)
        {
            auto.Click += (_, _) => ScheduleEndpointSelection(1800);
        }
    }
    ...
    int GetCurrentPid()
    {
        ...
        try { return Process.GetProcessesByName(Path.GetFileNameWithoutExtension(current.ProcessName)).FirstOrDefault()?.Id ?? 0; }
        catch { return 0; }
    }
    ...
    void StartLiveTracker(string ip, int port, string protocol)
    {
        ...
        polishTimer.Tag = $"{ip}|{port}|{protocol}";
    }
    ...
    async void PolishTimer_Tick(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (polishTimer.Tag is string target && target.Contains('|'))
            {
                var parts = target.Split('|');
                if (IPAddress.TryParse(parts[0], out var ip)) await SampleLivePing(ip);
            }
            graph.Invalidate();
    ...
    {
        if (Controls.OfType<TableLayoutPanel>().FirstOrDefault()?.Controls[2] is not TableLayoutPanel body || body.Controls.Count < 3) return;
        if (body.Controls[2] is not TableLayoutPanel right) return;
        var cards = right.Controls.OfType<GRLInfoCard>().ToList();
        if (cards.Count < 2) return;
        var n = NetworkProfileDetector.DetectAsync;
        var pulse = (Math.Sin(phase * 1.7) + 1) * .5;
        cards[0].Content = network.Text.Length > 0 && !network.Text.StartsWith("NETWORK:")
            ? network.Text.Replace(" | ", Environment.NewLine)
            : "LIVE TELEMETRY\r\nISP  •  scanning\r\nASN  •  scanning\r\nPUBLIC IP  •  scanning\r\nGATEWAY  •  scanning\r\nDNS  •  scanning";
        cards[1].Content = $"ROUTER LINK  •  {(pulse > .2 ? "ACTIVE" : "SYNC")}\r\nGateway  •  {(network.Text.Contains("GW ") ? network.Text.Split("GW ").LastOrDefault() : "scanning")}\r\nRoute state  •  monitoring\r\nConfidence  •  LIVE";
        cards[0].Invalidate();
        cards[1].Invalidate();
    }
    ...
    async Task SampleLivePing(IPAddress ip)
    {
        ...
        try
        {
            var reply = await p.SendPingAsync(ip, 850);
            if (reply.Status == System.Net.NetworkInformation.IPStatus.Success)
            {
                ...
                if (liveSamples.Count > 30) liveSamples.RemoveAt(0);
        ...
        catch { liveLost++; liveSamples.Add(-1); }

        if (liveSamples.Count > 30) liveSamples.RemoveAt(0);
        var successful = liveLatency.Count;
        var total = successful + liveLost;
        var avg = successful == 0 ? 0 : liveLatency.Average();
        var jitter = liveJitter.Count == 0 ? 0 : liveJitter.Average();
        var loss = total == 0 ? 0 : liveLost * 100.0 / total;
        var stability = loss == 0 && jitter <= 5 && avg > 0 && avg <= 80 ? "Excellent" : loss <= 2 && jitter <= 12 ? "Good" : avg > 0 ? "Fair" : "Waiting";

        metrics.Text = $"LATENCY     {(avg > 0 ? avg.ToString("0") : "—")} ms\r\nLOSS        {loss:0.#}%\r\nJITTER      {(jitter > 0 ? jitter.ToString("0.#") : "—")} ms\r\nSTABILITY   {stability}";
        quality.Text = successful == 0 ? "WAITING" : stability.ToUpperInvariant();
        quality.ForeColor = stability == "Excellent" ? Green : stability == "Good" ? Yellow : stability == "Waiting" ? Muted : Red;
        graph.Values = liveLatency.Count > 1 ? new List<double>(liveLatency) : new List<double> { avg > 0 ? avg : 0, avg > 0 ? avg : 0 };
        graph.Invalidate();
    }
}
