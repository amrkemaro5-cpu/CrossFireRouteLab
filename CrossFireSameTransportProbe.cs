...
internal static class CrossFireSameTransportProbe
{
    ...
    public static void Apply(GameRouteLabV10Form form)
    {
        ...
        Log(form, "[CROSSFIRE] Same-transport probe enabled: room traffic can be multiplexed on the channel/master TCP socket.");
    }
    ...
    static void Tick(GameRouteLabV10Form form)
    {
        ...
        _ = Task.Run(() => Probe(form, pid));
    }

    static async Task Probe(GameRouteLabV10Form form, int pid)
    {
        ...
        try
        {
            ...
            var local = LocalIPv4s();
            var flows = Parse(File.ReadAllBytes(pcap), local, endpoints);
            var best = flows.Where(x => x.Packets >= 3).OrderByDescending(x => x.Score).FirstOrDefault();
            if (best.Packets < 3)
            {
                Log(form, "[CROSSFIRE] No room packets were correlated during this probe window.");
                return;
            }
    ...
    static void Publish(GameRouteLabV10Form form, Flow f)
    {
        ...
        if (type.GetField("endpoint", flags)?.GetValue(form) is object)
        ...
        else Log(form, "[CROSSFIRE] UDP traffic is present on the same endpoint. No application RTT is claimed because UDP has no built-in ACK.");
    }
    ...
    static List<Flow> Parse(byte[] d, HashSet<string> local, List<Endpoint> known)
    {
        ...
        int o = 0; var interfaces = new HashSet<uint>();
        while (o + 12 <= d.Length)
        {
            uint type = LE32(d, o), len = LE32(d, o + 4);
            if (len < 12 || o + len > d.Length) break;
            if (type == 0x00000001) interfaces.Add((uint)interfaces.Count);
            else if (type == 0x00000006 && len >= 32)
            {
                uint id = LE32(d, o + 8), hi = BE32(d, o + 12), lo = BE32(d, o + 16), cap = LE32(d, o + 20);
                double ts = ((ulong)hi << 32 | lo) / 1_000_000.0;
                int po = o + 28;
                if (cap > 0 && po + cap <= o + len - 4)
                {
                    var p = ParsePacket(d, po, (int)cap, local, known, ts);
                    if (p != null)
                    {
                        ...
                        if (p.Rtt.HasValue) mf.Rtt.Add(p.Rtt.Value);
                        map[key] = mf;
                        if (p.Out && p.Protocol == "TCP" && p.Payload > 0) outgoing.Add(new Pending(key, p.SeqEnd, ts));
                        if (!p.Out && p.Protocol == "TCP" && p.Ack.HasValue)
                        {
                            for (int i = outgoing.Count - 1; i >= 0; i--)
                            {
                                if (outgoing[i].Key == key && p.Ack.Value >= outgoing[i].SeqEnd)
                                {
    ...
    static Packet? ParsePacket(byte[] d, int o, int len, HashSet<string> local, List<Endpoint> known, double ts)
    {
        if (len < 34 || BE16(d, o + 12) != 0x0800) return null;
        int ip = o, ihl = (d[ip] & 15) * 4; if (ihl < 20 || ip + ihl + 8 > o + len) return null;
        byte proto = d[ip + 9]; string src = new IPAddress(new ReadOnlySpan<byte>(d, ip + 12, 4)).ToString(), dst = new IPAddress(new ReadOnlySpan<byte>(d, ip + 16, 4)).ToString();
        ...
        if (proto == 17) return new Packet(rip, rp, "UDP", outb, len, 0, 0, 0, null);
        if (proto != 6 || tr + 20 > o + len) return null;
        uint seq = BE32(d, tr + 4), ack = BE32(d, tr + 8); int thl = ((d[tr + 12] >> 4) & 15) * 4; int payload = Math.Max(0, len - ihl - thl);
        bool syn = (d[tr + 13] & 2) != 0, fin = (d[tr + 13] & 1) != 0; uint seqEnd = seq + (uint)payload + (syn ? 1u : 0u) + (fin ? 1u : 0u);
        return new Packet(rip, rp, "TCP", outb, len, payload, seqEnd, ack, null);
    }

    static HashSet<string> LocalIPv4s()
    {
        var s = new HashSet<string>(StringComparer.OrdinalIgnoreCase); foreach (var n in System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces()) foreach (var a in n.GetIPProperties().UnicastAddresses) if (a.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork) s.Add(a.Address.ToString()); return s;
    }
    ...
    readonly record struct Packet(string RemoteIp, int RemotePort, string Protocol, bool Out, int Bytes, int Payload, uint SeqEnd, uint Ack, double? Rtt);
    readonly record struct RunResult(int Code);
    sealed class MutableFlow { public string Ip, Protocol; public int Port, Packets, Inbound, Outbound; public long Bytes; public List<double> Rtt = new(); public MutableFlow(string ip,int port,string protocol){Ip=ip;Port=port;Protocol=protocol;} public Flow ToFlow()=>new(Ip,Port,Protocol,Packets,Inbound,Outbound,Bytes,Rtt,Packets+Rtt.Count*20); }
