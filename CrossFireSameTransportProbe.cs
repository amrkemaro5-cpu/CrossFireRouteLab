...
internal static class CrossFireSameTransportProbe
{
    ...
    static async Task Probe(GameRouteLabV10Form form)
    {
        ...
        try
        {
            ...
            etl = Path.Combine(root, $"same-{stamp}.etl");
            pcap = Path.Combine(root, $"same-{stamp}.pcapng");
            ...
            if (start.Code != 0)
            {
                Log(form, "[CROSSFIRE] Same-transport packet probe could not start; run GRL as Administrator.");
                return;
            }
        ...
        {
            Log(form, "[CROSSFIRE] Same-transport probe error: " + ex.Message);
        }
        finally
        {
    ...
    static void Publish(GameRouteLabV10Form form, Flow f)
    {
        ...
        if (f.Rtt.Count > 0)
            Log(form, $"[CROSSFIRE] TCP packet RTT estimate = {Median(f.Rtt):0} ms from live ACK timing. This is transport RTT, not an ICMP ping.");
        else
            Log(form, "[CROSSFIRE] UDP traffic is present on the same endpoint. UDP has no built-in ACK, so GRL does not invent an application RTT.");
    }
    ...
    static HashSet<string> LocalIPv4s()
    {
        ...
        foreach (var n in System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces())
            foreach (var a in n.GetIPProperties().UnicastAddresses)
                if (a.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork) s.Add(a.Address.ToString());
        return s;
    }
    ...
    sealed class MutableFlow
    {
        ...
        public MutableFlow(string ip, int port, string protocol)
        {
            Ip = ip;
            Port = port;
            Protocol = protocol;
