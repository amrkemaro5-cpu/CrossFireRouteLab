...
internal static class CrossFirePacketRoomDiscoveryPatchV2
{
    ...
    public static void Apply(GameRouteLabV10Form form)
    {
        ...
        if (!IsAdministrator())
        {
            Log(form, "[CROSSFIRE] Hidden room discovery needs Game Route Lab to run as Administrator for Windows PktMon capture.");
            warnedElevation = true;
        }
        else Log(form, "[CROSSFIRE] Packet-level CrossFire room discovery enabled.");
    }
    ...
    static void Tick(GameRouteLabV10Form form)
    {
        ...
        if (!IsAdministrator())
        {
            if (!warnedElevation) { Log(form, "[CROSSFIRE] Restart Game Route Lab as Administrator so packet discovery can inspect the hidden room transport."); warnedElevation = true; }
            return;
        }
        if (DateTime.UtcNow - lastCapture < TimeSpan.FromSeconds(12)) return;
        var current = ReadCurrentEndpoints(form);
        if (current.Count > 1 || !current.Any(x => x.Port is 10009 or 13008)) return;
    ...
    static List<Endpoint> ReadCurrentEndpoints(GameRouteLabV10Form form)
    {
        ...
        try
        {
            ...
            foreach (var item in list)
            {
                ...
                if (IPAddress.TryParse(ip, out _)) result.Add(new Endpoint(ip, port, protocol));
            }
        }
        catch { }
        return result;
    ...
    static async Task CaptureAndPublish(GameRouteLabV10Form form, List<Endpoint> known)
    {
        ...
        try
        {
            Log(form, "[CROSSFIRE] Capturing live NIC traffic for 6 seconds to find the actual room transport…");
            ...
            if (start.ExitCode != 0)
            {
                Log(form, "[CROSSFIRE] PktMon could not start. Make sure Game Route Lab is running as Administrator.");
                return;
            }
            await Task.Delay(6000).ConfigureAwait(false);
            await RunAsync("pktmon.exe", "stop").ConfigureAwait(false);
            if (!File.Exists(etl)) { Log(form, "[CROSSFIRE] No PktMon capture file was produced."); return; }
            var convert = await RunAsync("pktmon.exe", "etl2pcap", etl, "--out", pcap).ConfigureAwait(false);
            if (convert.ExitCode != 0 || !File.Exists(pcap)) { Log(form, "[CROSSFIRE] PktMon capture could not be converted to PCAPNG."); return; }
            ...
            var candidates = flows.Where(x => !knownKeys.Contains($"{x.RemoteIp}:{x.RemotePort}"))
                .Where(IsUsefulCandidate).OrderByDescending(Score).Take(12).ToList();
            if (candidates.Count == 0)
            {
                Log(form, "[CROSSFIRE] Packet capture found no additional public room-flow candidate. The room may be using a transport hidden below the normal socket table.");
                return;
            }
            Publish(form, candidates);
        }
        catch (Exception ex) { Log(form, "[CROSSFIRE] Packet discovery error: " + ex.Message); }
        finally { captureRunning = false; TryDelete(etl); TryDelete(pcap); }
    }
    ...
    static bool IsUsefulCandidate(PacketFlow f)
    {
        ...
        if (f.RemotePort < 1024 && f.RemotePort is not 10009 and not 13008 and not 16666) return false;
        return true;
    }
    ...
    static void Publish(GameRouteLabV10Form form, List<PacketFlow> candidates)
    {
        ...
        form.BeginInvoke((Action)(() =>
        {
            try
            {
                ...
                Log(form, $"[CROSSFIRE] Packet discovery found {candidates.Count} additional room-flow candidate(s): {candidates.Count(x => x.Protocol == "TCP")} TCP, {candidates.Count(x => x.Protocol == "UDP")} UDP.");
    ...
    static List<PacketFlow> ParsePcap(string path, HashSet<string> localIps)
    {
        ...
        while (offset + 12 <= data.Length)
        {
            ...
            {
                interfaces[(uint)interfaces.Count] = U16(data, offset + 8);
            }
            else if (blockType == 0x00000006 && blockLength >= 32)
            {
                ...
                if (captured > 0 && packetOffset + captured <= offset + blockLength - 4 && interfaces.TryGetValue(id, out var linkType))
                {
                    ...
                    if (parsed is PacketFlow flow)
                    {
                        ...
                        if (flows.TryGetValue(key, out var old)) flows[key] = old with { Packets = old.Packets + 1, Bytes = old.Bytes + flow.Bytes, Inbound = old.Inbound + flow.Inbound, Outbound = old.Outbound + flow.Outbound };
                        else flows[key] = flow;
                    }
                }
            }
    ...
    static PacketFlow? ParsePacket(byte[] d, int o, int len, ushort linkType, HashSet<string> localIps)
    {
        ...
        int srcPort = U16(d, transport), dstPort = U16(d, transport + 2);
        ...
        if (outbound) { remote = dst; port = dstPort; }
